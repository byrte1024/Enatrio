...
// Allocates a new array -- caller must destroy the result.
...
// Allocates a new array with transformed elements -- caller must destroy the result.
...
// Returns pointer into the array's own storage -- do not free.
...
// Uses memcmp for equality -- works for any flat type, not for pointer-based structs.
...
// Folds all elements into caller-initialized accumulator.
...
// Returns pointer into the array's storage, or NULL if empty.
...
// Mutating iteration -- action receives a mutable pointer.
...
// Order-preserving compaction -- shifts survivors down.
...
// Fisher-Yates shuffle -- call srand() beforehand for varied results.
...
// Order-preserving deduplication via memcmp. O(n^2) -- fine for small arrays.
...
// Both arrays must have the same element_size. Caller must destroy the result.
