...
// CAT macros perform direct token pasting (no expansion of macro arguments).
// BAT macros force macro expansion before pasting, since the C preprocessor
// does not expand arguments that appear next to ## in the same macro body.
#define CAT1(a) a
#define CAT2(a,b) a##b
...
// STR stringifies literally; BSTR expands macros first via the extra indirection
#define STR(x) #x
#define BSTR(x) STR(x)
...
// Newlines and ANSI escapes in log messages would corrupt log file structure
...
// LOG_TRACE intentionally maps to LOG_INFO -- raylib filters out TRACE by default
...
#else
    // XDG_DATA_HOME takes precedence per the XDG Base Directory Specification
